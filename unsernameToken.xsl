...
<xsl:stylesheet version="1.0"
	...
	<!-- Variables globales -->
	 <xsl:param name="dpquery:USERNAME" select="''"/>
     <xsl:param name="dpquery:PASSWORD" select="''"/>
	...
	<xsl:variable name="username">
		...
		<xsl:variable name="value" select="$dpquery:USERNAME"/>
		<xsl:choose>
			<xsl:when test="$value != ''">
			...
			<xsl:otherwise>
				<dp:reject>
					...
					requires UsernameToken Injection but no user has been specified.
	...
	<xsl:variable name="password">
		...
		<xsl:variable name="value" select="$dpquery:PASSWORD"/>
		<xsl:choose>
			<xsl:when test="$value != ''">
				<xsl:value-of select="$value" />
			</xsl:when>
